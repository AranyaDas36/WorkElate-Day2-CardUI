...
export function Card({ task, onUpdate, onDelete, showToast }) {
  ...
  return (
    <>
      ...
      >
        <h3 className="text-xl font-bold mb-1 text-teal-700">{task.title}</h3>
        ...
        <div className="flex gap-2 mt-2">
          <button
            ...
            className="px-4 py-1 rounded-lg bg-teal-700 text-white hover:bg-teal-900 transition font-medium shadow"
          >
            Edit
          </button>
          <button
            onClick={e => { stopPropagation(e); handleDelete(); }}
            className="px-4 py-1 rounded-lg bg-gray-300 text-black hover:bg-gray-400 transition font-medium shadow"
          >
            Delete
          </button>
        </div>
